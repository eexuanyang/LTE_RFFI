...
def save_logs(output_directory, hist, y_pred, y_true, duration, lr=True, y_true_val=None, y_pred_val=None, trip = False):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)
    ...
    if trip == False:
        ...
        df_best_model['best_model_train_acc'] = row_best_model['dense_accuracy']
        df_best_model['best_model_val_acc'] = row_best_model['val_dense_accuracy']
